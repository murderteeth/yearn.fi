...
import { localhost } from '@yearn/deposit/utils/wagmi/networks'
...
export function getRpcUriFor(chainId: number | string): string {
  ...
  if (Number.isInteger(normalizedChainId)) {
    const tenderlyRpc = depositConfig().forkedRpcUri(normalizedChainId)
    if (tenderlyRpc) {
      return tenderlyRpc
    }
  }
...
function toExtendedChain(chain: Chain): TExtendedChain {
  ...
  const oldRPC =
    process.env.NEXT_PUBLIC_JSON_RPC_URI?.[extendedChain.id] || process.env.NEXT_PUBLIC_JSON_RPC_URL?.[extendedChain.id]
  if (!newRPC && oldRPC) {
    console.debug(
  ...
  const tenderlyExecutionExplorer = depositConfig().forkedExplorerUri(extendedChain.id)
  const isTenderlyExecutionChain = Boolean(depositConfig().forkedRpcUri(extendedChain.id))
...
function initIndexedWagmiChains(): TNDict<TExtendedChain> {
  ...
  const baseChains = Object.values({ ...wagmiChains, rari, localhost }).filter(isChain)

  for (const chain of [...baseChains, ...depositConfig().chains]) {
...
export function getClient(chainID: number): PublicClient {
  ...
  const oldRPC =
    process.env.NEXT_PUBLIC_JSON_RPC_URI?.[executionChainId] || process.env.NEXT_PUBLIC_JSON_RPC_URL?.[executionChainId]
