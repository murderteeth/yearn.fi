...
import { depositConfig } from '@yearn/deposit/config'
import { retrieveConfig } from '@yearn/deposit/utils/wagmi/config'
import type { TWriteTransaction } from '@yearn/deposit/utils/wagmi/provider'
import { handleTx, toWagmiProvider } from '@yearn/deposit/utils/wagmi/provider'
import type { TTxResponse } from '@yearn/deposit/utils/wagmi/transaction'
...
import { PARTNER_VAULT_ABI } from '@yearn/vaults/abi/partner.vault.abi'
import { VAULT_ABI } from '@yearn/vaults/abi/vault.abi'
import { VAULT_MIGRATOR_ABI } from '@yearn/vaults/abi/vaultMigrator.abi'
import { VAULT_V3_ABI } from '@yearn/vaults/abi/vaultV3.abi'
import { ZAP_ETH_TO_YVETH_ABI } from '@yearn/vaults/abi/zapEthToYvEth.abi'
import { ZAP_FTM_TO_YVFTM_ABI } from '@yearn/vaults/abi/zapFtmToYvFTM.abi'
import { ZAP_OPT_ETH_TO_YVETH_ABI } from '@yearn/vaults/abi/zapOptEthToYvEth'
import { getEthZapperContract } from '@yearn/vaults/utils/nativeTokens'
import { erc20Abi } from 'viem'
import type { Connector } from 'wagmi'
import { readContract } from 'wagmi/actions'

function getExecutionChainID(chainID: number): number {
  const executionChainId = depositConfig().toExecutionChainId(chainID)
...
export async function depositViaPartner(props: TDepositViaPartner): Promise<TTxResponse> {
  ...
  return await handleTx(props, {
    ...
    args: [
      props.vaultAddress,
      props.partnerAddress || toAddress(process.env.NEXT_PUBLIC_PARTNER_ID_ADDRESS),
      props.amount
    ]
  })
}
