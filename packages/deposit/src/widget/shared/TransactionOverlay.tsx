...
import { useNotificationsActions } from '@yearn/deposit/contexts/useNotificationsActions'
import {
  ...
  const executeContractStep = useCallback(
    async (currentStep: TransactionStep) => {
      if (currentStep.batch) {
        ...
        if (!depositConfig().isConnectedToExecutionChain(connectedChainId, txChainId)) {
          try {
      ...
      const wrongNetwork = txChainId && !depositConfig().isConnectedToExecutionChain(connectedChainId, txChainId)
