...
import { useMerkleRewards } from '@yearn/deposit/hooks/rewards/useMerkleRewards'
import { type TRewardToken, useStakingRewards } from '@yearn/deposit/hooks/rewards/useStakingRewards'
import { useChainId, useSwitchChain } from '@yearn/deposit/hooks/useAppWagmi'
import { MerkleRewardRow } from '@yearn/deposit/widget/rewards/MerkleRewardRow'
import { StakingRewardRow } from '@yearn/deposit/widget/rewards/StakingRewardRow'
...
export function WidgetRewards(props: TWidgetRewardsProps): ReactElement | null {
  ...
  const { address: userAddress, isActive } = useYearnWallet()
  ...
  const handleClaimComplete = useCallback(() => {
    trackEvent(WIDGET_EVENTS.CLAIM, {
      props: {
          ...
          <div className="flex-1 min-h-0 border-t border-border px-6 py-4 overflow-y-auto">
            {isComplete ? (
              ...
              <div>
                ...
                {!isOnCorrectChain && (
                  <SwitchChainPrompt
                    chainName={depositConfig().chains.find((n) => n.id === chainId)?.name}
                    chainId={chainId}
