...
import { useChainId, useSwitchChain, useWaitForTransactionReceipt } from '@yearn/deposit/hooks/useAppWagmi'
import { useSafeTransactionDetails } from '@yearn/deposit/hooks/useSafeTransactionDetails'
import {
  resolveApprovalOverlayActionDisabledState,
  resolveApprovalOverlayConnectedChainId,
  resolveApprovalOverlayPendingSafeState
} from '@yearn/deposit/widget/deposit/ApprovalOverlay.helpers'
import { InfoOverlay } from '@yearn/deposit/widget/shared/InfoOverlay'
import { AnimatedCheckmark, ErrorIcon, Spinner } from '@yearn/deposit/widget/shared/TransactionStateIndicators'
import {
  resolveExecutionTrackingHash,
  resolveTransactionReceiptOutcome
} from '@yearn/deposit/widget/shared/transactionOverlay.helpers'
import { isSafeConnectorId } from '@yearn/util'
...
export const ApprovalOverlay: FC<ApprovalOverlayProps> = ({
  ...
  const handleApprove = useCallback(
    async (amount: bigint) => {
      ...
      if (!depositConfig().isConnectedToExecutionChain(connectedChainId, chainId)) {
        try {
