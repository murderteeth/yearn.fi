...
import { type EnsoRouteResponse, normalizeEnsoRouteResponse } from '@yearn/deposit/hooks/solvers/ensoRoute'
import { calculateDepositValueInfo, resolveValuationShareCount } from '@yearn/deposit/widget/deposit/valuation'
      ...
      if (stakingAddress && previewRedeemCall) {
        try {
          const normalized = await readContract(config, {
            chainId: depositConfig().toExecutionChainId(chainId) ?? chainId,
