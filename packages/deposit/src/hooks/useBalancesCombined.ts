...
import { shouldUseDiscoveryFallbackToken } from '@yearn/deposit/hooks/balanceDiscoveryFallback'
import {
...
} from '@yearn/deposit/hooks/useBalances.multichains'
import { fetchTokenBalances, useBalancesQueries } from '@yearn/deposit/hooks/useBalancesQueries'
import { balanceQueryKeys } from '@yearn/deposit/hooks/useBalancesQuery'
import { partitionTokensByBalanceSource } from '@yearn/deposit/hooks/useBalancesRouting'
import { ENSO_UNSUPPORTED_NETWORKS, useEnsoBalances } from '@yearn/deposit/hooks/useEnsoBalances'
...
export function useBalancesCombined(props?: TUseBalancesReq): TUseBalancesRes {
  const { address: userAddress } = useYearnWallet()
  const queryClient = useQueryClient()
  const isTenderlyMode = depositConfig().isForkedNetwork()
