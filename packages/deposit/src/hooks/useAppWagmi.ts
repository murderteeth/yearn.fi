...
function resolveHookChainId(chainId?: number): number | undefined {
  ...
  const resolvedChainId = depositConfig().toExecutionChainId(chainId)
  if (resolvedChainId !== undefined) {
    return resolvedChainId
  }

  return depositConfig().isForkedNetwork() ? DISABLED_CHAIN_ID : chainId
}

function isUnsupportedRequestedChain(chainId?: number): boolean {
  return Number.isInteger(chainId) && depositConfig().toExecutionChainId(chainId) === undefined
}
...
export function useChainId(): number {
  ...
  return depositConfig().toCanonicalChainId(rawChainId) ?? (depositConfig().isForkedNetwork() ? 0 : rawChainId)
}
...
export function useSwitchChain(): UseSwitchChainReturnType {
  ...
  const switchChain = useCallback<UseSwitchChainReturnType['switchChain']>(
    (parameters) => {
      const executionChainId = depositConfig().toExecutionChainId(parameters.chainId)
