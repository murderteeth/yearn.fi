...
import { getStakingWithdrawableAssets } from '@yearn/deposit/hooks/actions/stakingAdapter'
...
import { getVaultChainID, getVaultStaking, type TKongVault } from '@yearn/vaults/domain/kongVaultSelectors'
    ...
    queries: stakingPositions.map((position) => ({
      ...
      queryFn: async () => {
        ...
        const read = (request: {
          ...
          const executionChainId = depositConfig().toExecutionChainId(position.chainID)
          if (!executionChainId) {
      ...
      enabled: Boolean(
        userAddress && position.stakingShareBalance > 0n && depositConfig().toExecutionChainId(position.chainID)
      ),
      staleTime: 60_000,
      gcTime: 5 * 60_000,
      retry: 1,
