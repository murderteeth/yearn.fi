...
function StaticRiskScore(): ReactElement {
  ...
  return (
    <div className={'grid grid-cols-1 gap-4 p-4 pt-0 md:grid-cols-12 md:gap-10 md:p-6 md:pt-0'}>
      <div className={'col-span-12 w-full space-y-1'}>
        {YVBTC_RISK_SCORE_ITEMS.map((item, index) => (
          <RiskScoreItem
            key={item.label}
            label={item.label}
            score={item.score}
...
function SimpleRiskScore({
  ...
  const [openIndex, setOpenIndex] = useState<number | null>(null)
  const info = getVaultInfo(currentVault)
  const isMultiStrategy = getVaultKind(currentVault)?.toLowerCase() === 'multi strategy'

  const toggleItem = (index: number): void => {
    setOpenIndex(openIndex === index ? null : index)
  }
  const renderInlineRiskScoreTag = (riskLevelOverride?: number): ReactElement => {
    const riskLevel = riskLevelOverride ?? info.riskLevel
  ...
  const riskScoreData = [
    {
      label: 'Overall Risk Score',
      score: info.riskLevel,
      explanation: info.riskScoreComment,
      isOverall: true,
      rightContent: renderInlineRiskScoreTag()
    },
    {
      label: 'Review',
      score: info.riskScore[0],
      explanation:
